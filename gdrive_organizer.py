...
class DriveManager:
    ...
    def __repr__(self):
        ...
        return f"<DriveManager service_ready={self.service is not None} user='{self.user_info or 'Unauthenticated'}' >"
    ...
    def display_user_info(self):
        """Prints the logged-in user's email."""
        if self.service and self.user_info:
            print(f"\n✅ Logged in as: {self.user_info}")
        else:
    ...
    def _build_tree(self, files):
        ...
        This is done in two phases to avoid KeyErrors due to file order.
        """
        file_map = {file['id']: file for file in files}
        tree = {}

        # Phase 1: Enrich file objects with custom properties like 'is_folder' and 'children'
        for file in files:
            file['children'] = []
            # 'is_folder' is set here, ensuring it exists for later lookups
            file['is_folder'] = file['mimeType'] == 'application/vnd.google-apps.folder'
        
        # Phase 2: Link children to their parents
        for file in files:
            parents = file.get('parents', [])
            is_attached = False
            
            for parent_id in parents:
                # Safely check if the parent exists in the fetched list and is a folder
...
def main_menu():
    ...
    if hasattr(manager, 'display_user_info'):
        manager.display_user_info()
    else:
        # Fallback if the method still fails (e.g., if the user manually edited the class definition)
        print("\n⚠️ Warning: Could not find 'display_user_info' method, but connection seems functional.")
